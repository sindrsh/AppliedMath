...
\reg[Sannsynlighet for en hendelse \label{grnprsp}]{
\[ \text{sannsynlighet for en hendelse}=\frac{\text{antall gunstige utfall}}{\text{antall mulige utfall}} \]
} \vsk
...
$ P(A) $ er sannsynligheten for at hendelsen $ A $	inntreffer. \vsk

$ A $ og $ \bar{A} $ er motsatte hendelser. \vsk

$ P(\bar{A}) $ er sannsynligheten for at $ A $ \textsl{ikke} inntreffer, og omvendt.
}
\info{Obs!}{
...
\prbxl{0.6}{La oss kalle hendelsen å 'å få en treer' (på en terning) for \textit{T}. Hendelsen 'å få en treer \textsl{eller} en firer' skriver vi da som $ T\cup F $.} \qquad
...
Enten så skjer $ F $, eller så skjer den ikke. Og skjer den ikke, så skjer $ \bar{F} $. Hvis vi sier at \textsl{både} $ F $ og $ \bar{F} $ er gunstige hendelser, sier vi altså at alle mulige utfall er gunstige, og da gir  \rref{grnprnsp} en sannsynlighet lik 1. \regv
...
\reg[Motsatte hendelser \label{motsatt}]{\vs
\[ P(A)=1-P(\bar{A}) \]\vs\vs
}
\eks{
...
\prbxl{0.5}{Tenk at vi trekker opp et kort fra en blandet kortstokk. 
	Vi ønsker å finne sannsynligheten for 'å trekke kløver \textsl{eller} honnør \textsl{eller} begge deler'.
...
Så vi har altså telt opp ${13+16=29}$ gunstige utfall. Men nå støter vi på et problem. For da vi fant alle kløverkort, telte vi blant andre kløver knekt, dame, kong og ess. Disse fire kortene telte vi også da vi fant alle honnørkort, noe som betyr at vi har telt de samme kortene to ganger! \\
	...
	\end{figure}
Det finnes jo for eksempel ikke to kløver ess i en kortstokk, så skal vi regne ut hvor mange kort som oppfyller kravet om å være kløver, honnør \textsl{eller} begge deler $ - $ ja, så må vi trekke ifra antallet kort vi har telt dobbelt:
$$ 13+16-4=25 $$
	\begin{figure}[H]
		\centering
...
I en klasse på 20 personer spiller 7 personer fotball og 10 personer spiller handball. Av disse er det 4 som spiller både fotball og handball. Tenk deg at du trekker ut én person fra klassen. Hva er sjansen for at denne personen spiller fotball \textsl{eller} handball?
...
Målet med et venndiagram er å lage en figur som illustrerer antallet av de enkelte utfallene og de \textit{felles} utfallene. La oss bruke eksempelet på side \pageref{eksfothand} til å lage en slik figur. For klassen der noen spiller fotball, noen handball og noen begge deler, kan vi lage et venndiagram som vist under.
\begin{figure}
	\centering
...
\end{figure}
Diagrammet forteller nå at 3 personer spiller \textsl{bare} fotball og 6 spiller \textsl{bare} handball. I tilleg spiller 4 stykker \textsl{både} fotball og handball. (Til sammen er det derfor 7 som spiller fotball og 10 som spiller handball.) \regv
...
\begin{figure}[H]
	...
	\vs
\end{figure}
La oss tenke oss at vi har en bolle med fire kuler som er nummererte fra 1 til 4. I et forsøk trekker vi opp én og én kule fram til vi har trukket opp tre kuler. Hvis vi for eksempel først trekker kule 2, deretter kule 4, og så kule 3, får vi \textit{permutasjonen} $2\; 4\; 3$. \vsk
...
\[ 4\cdot3\cdot2=24 \]
\reg[Permutasjoner]{Når vi foretar flere trekninger etter hverandre, finner vi alle mulige permutasjoner ved å gange sammen de mulige utfallene i hver trekning.
}
...
\end{figure}
Tenke at vi har en med bolle sju kuler. Tre av dem er grønne, to er blå og to er røde. Si at vi tar opp først én kule av bollen, og deretter én til. Vi spør oss nå hva sannsynligheten er for at vi trekker opp to grønne kuler. Hvis vi lar $ G $ bety 'å trekke en grønn kule', kan vi skrive denne sannsynligheten som $ P(GG) $.\vsk
...
\end{equation}
\rule{\linewidth}{1pt}
La oss nå i hver trekning se på sannsynligheten for å få en grønn kule, med krav om at dette skal skje i begge trekninger. I første trekning har vi 3 grønne av i alt 7 kuler, altså er
\[ P(G)=\frac{3}{7} \]
...
Hvis vi ganger sammen sannsynligheten fra første trekning, med sannsynligheten fra andre trekning, ser vi at regnestykket blir det samme som i ligning \eqref{trekk}:
...
\eks{
	I en bolle ligger to blå og to røde kuler.  Vi trekker én og én kule opp av bollen, fram til vi har hentet opp tre kuler. Hva er sannsynligheten for at vi først trekker en blå kule, deretter en rød, og til slutt en blå? 
