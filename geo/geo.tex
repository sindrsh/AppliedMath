...
\newpage
\info{Tips}{Det er ikke så lett å se for seg hva en \textsl{sammenbrettet sektor} er, men prøv dette: 
\begin{enumerate}
	\item Tegn en sektor på et ark. Klipp ut sektoren, og føy sammen de to kantene på sektoren. Da har du en kjegle uten bunn.
\end{enumerate}
}
\section{Volum}
Når vi ønsker å si noe om hvor mye det er plass til inni en gjenstand, snakker vi om \textit{volumet} tilden. Som et mål på volum tenker vi oss en kube med sidelengde 1.
\fig{vol1}
...
For å regne ut volumet til de mest elementære figurene vi har, kan det være lurt å bruke begrepet \textit{grunnflate}. Slik som for en grunnlinje\footnote{se \mb, s. 81.}, er det vårt valg av grunnflate som avgjør hvordan vi skal regne ut høgden. For prismen fra forrige side, er det naturlig å velge flaten som ligger horisontalt til å være grunnflaten, og for å indikere dette skrives ofte bokstaven $ G $:
\fig{vol2b}
Grunnflaten har arealet $ 3\cdot4=12 $, mens høgden er 2. Volumet til hele prismen er grunnflatens areal ganget med høgden:
\alg{
	 V &= 3\cdot 4 \cdot 2 \\
	 &= G\cdot 2\\
	 &= 24
...
I teksten over har vi først kalt selve grunnflaten for $ G $, og så brukt $ G $ for \textsl{grunnflatearealet}. I denne boka er begrepet \textsl{grunnflate} så sterkt knyttet til \textsl{grunnflatearealet} at vi ikke skiller mellom disse to.
}
...
\sv
Så lenge vi er sikre på at størrelsene vår har samme benevning (i dette tilfellet 'm'), kan vi først rekne uten størrelser:
\alg{
