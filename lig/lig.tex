...
\info{Tips}{
Ved å la enkeltbokstaver representere størrelser, får man kortere uttrykk. La $ k $ stå for 'kostnad for taxituren' og $ x $ stå for 'antall kilometer kjørt'. Da blir uttrykket fra \textsl{Eksempel 1} over dette:
\[ k=50+15x \]
...
\eks[1]{
	...
	\[ s = f\cdot t \] 
	Dett er altså en formel for $ s $. Ønsker vi i stedet en formel for $ f $, kan vi gjøre om formelen ved å følge prinsippene for likninger (se \mb, s. ??):
	\alg{
		s &= f\cdot t \br
...
\eks[2]{
	...
	\sv
	\textbf{a)} Vi gjør om formelen slik at $ R $ står alene på én side av likhetstegnet:\vs
	\alg{
...
\eks[3]{
	Si vi har målt en temperatur $ T_C $ i antall grader Celsius ($ ^\circ C $). Temperaturen $ T_F $ målt i antall grader Fahrenheit ($ ^\circ F $) er da gitt ved formelen
...
\abc{
\item Lag en ligning for hvor mange lodd klassen må selge for å få råd til klasseturen.
\item Løs ligningen.
}
...
\textit{Denne seksjonen tar utgangspunkt i at leseren er kjent med funksjoner, se \mb, s. ??.}
