...
Det er ikke noen fullstendige fasitsvar på hvordan man presenterer eller tolker data, men to retningslinjer bør du alltid ta med deg:
\begin{itemize}
	\item La det alltid komme tydelig fram hva du har undersøkt og hvilke data som er skaffet.
...
Mobiltelefoner med smartfunksjoner (app-baserte) kom på det norske markedet i 2009. Tabellen\footnote{Tallene er hentet fra \net{https://www.medienorge.uib.no/statistikk/medium/ikt/405}{medienorge.uib.no}.} under viser det totale salget mobiltelefoner i tidsperioden 2009-2014 og andelen med og uten smartfunkskjoner.
\begin{center}
...
Skal vi presentere våre undersøkelser, bør vi vise datasett slik at det er lett for andre å se hva vi har funnet. Dette kan vi gjøre blant annet ved hjelp av \textit{frekvenstabeller}, \textit{søylediagram}, \textit{sektordiagram} eller \textit{linjediagram}.

\subsection{Frekvenstabell}
I en frekvenstabell setter man opp dataene i en tabell som viser hvor mange ganger hver unike data dukker opp. Dette antallet kalles\\ \textit{frekvensen}.\regv
...
Skal du selv tegne et sektordiagram, trenger du kunnskaper om vinkler og om brøkandeler. Se s. ??, \mb, s. ?? og oppgave \ref{lagsekt}.}
\newpage
...
I datasett vil det ofte være svar som er helt eller veldig like, og som gjentar seg. Dette betyr at vi kan si noe om hva som gjelder for mange. De matematiske begrepene som forteller noe om dette kalles \textit{sentralmål}. De vanligste sentralmålene er \textit{typetall}, \textit{gjennnomsnitt} og \textit{median}. \vsk

\subsection{Typetall}
\reg[Typetall]{
Typetallet er verdien det er flest eksemplarer av i datasettet.
} \regv
\st{
\textbf{Undersøkelse 1} \os
...
Når et datasett består av svar i form av tall kan vi finne summen av svarene. Når vi spør oss hva gjennomsnittet er, spør vi om dette: \os
\textsl{''Hvis alle svarene var like, men summen den samme, hvilken verdi måtte alle svarene da ha hatt?''}\os

Dette er jo ingenting annet enn divisjon (se \mb, s. 23): \regv
...
\normalsize
\textit{Metode 2} \os
Vi utvider frekvenstabellen fra side ?? for å finne summen av verdiene fra datasettet (vi har også tatt med summen av frekvensene):
\begin{center}
	\begin{tabular}{|c|c|c|}
		\hline
...
\subsection{Median}
\reg[Median]{Medianen er tallet som ender opp i midten av datasettet når det rangeres fra tallet med lavest til høyest verdi.\vsk
...
Ofte vil det også være store forskjeller (stor spredning) mellom dataene som er samlet inn. De matematiske begrepene som forteller noe om dette er \textit{variasjonsbredde}, \textit{kvartilbredde}, \textit{varians} og \textit{standardavvik}.
...
\st{
	\textbf{Undersøkelse 4}
	\begin{itemize}
		\item Variasjonsbredde for mobiltelefoner:
		\[ 2\,500-2\,100=400  \]
		\item Variasjonsbredde for mobiltelefoner uten smartfunksjoner:
		\[ 1\,665-147=518 \]
		\item Variasjonsbredde for mobiltelefoner med smartfunksjoner:
		\[ 2\,160-700=1460 \]
	\end{itemize}
}
...
\begin{enumerate}
	\item Ranger datasettet fra høyest til lavest.
...
\end{enumerate}
Om medianene fra punkt 3: Den med høyest verdi kalles \textit{øvre kvartil} og den med lavest verdi kalles \textit{nedre kvartil}.
} \regv
\st{
\textbf{Undersøkelse 1} \os
\begin{enumerate}
	...
	\[ \color{blue}38\quad40\quad{\color{black}47}\quad 56\quad 61 \qquad\quad \color{red}79\quad 84\quad{\color{black}97}\quad 101\quad 124 \]
...
(Utregning utelatt)
\begin{itemize}
	\item For mobiltelefoner er kvartilbredden: 200 
	\item For mobiltelefoner uten smartfunksjoner er kvartilbredden: 1010
	\item For mobiltelefoner med smartfunksjoner er kvartilbredden: 703
\end{itemize}
} \vsk

\spr{
Nedre kvartil, medianen og øvre kvartil blir også omtalt som henholdsvis \textit{1. kvartil}, \textit{2. kvartil} og \textit{3. kvartil}.
}
...
\eks{ \label{vareks}
Gitt datasettet 
\[\color{blue} 2\quad 5\quad 9\quad 8\quad 10 \]
Da har vi at
\[ \text{gjennomsnitt}=\frac{\colb{2+5+9+8+10}}{5}=\colr{6} \]
Og videre er
\alg{
\text{variansen}&= \frac{(\colb{2}-\colr{6})^2+(\colb{5}-\colr{6})^2+(\colb{9}-\colr{6})^2+(\colb{8}-\colr{6})^2+(\colb{8}-\colr{6})^2}{5} \\ 
&= 6,8
}
Da er $ \text{standardavviket}=\sqrt{6,8}\approx2,6 $.
}
\st{
\textbf{Undersøkelse 1} \os
...
\st{
\textbf{Undersøkelse 2} \os
Gjennomsnittet fant vi på side \pageref{gjsnund2}. Vi utvider frekvenstabellen vår fra side ??:
\begin{center}
...
\st{
	...
	(Utregning utelatt)
	\begin{itemize}
		\item For mobiltelefoner er variansen 17\,781,25 
		\item For mobiltelefoner uten smartfunksjoner er variansen $ 318\,848.\bar{3} $ 
		\item For mobiltelefoner med smartfunksjoner er variansen $245\,847.91\bar{6} $
	\end{itemize}
}
\info{Hvorfor innebærer variansen kvadrering?}{
La oss se hva som skjer hvis vi gjentar utregningen fra \textsl{Eksempel 1} på side \pageref{vareks}, men uten å kvadrere:
\begin{multline}
	\frac{(\colb{2}-\colr{6})+(\colb{5}-\colr{6})+(\colb{9}-\colr{6})+(\colb{6}-\colr{6})+(\colb{8}-\colr{6})}{5}\\=\frac{\colb{2+5+9+6+8}}{5} - \colr{6}
\end{multline}
Men brøken $ \frac{\colb{2+5+9+6+8}}{5} $ er jo per definisjon gjennomsnittet til datasettet, og dermed blir uttrykket over lik 0. Dette vil gjelde for alle datasett, så i denne sammenhengen gir ikke tallet 0 noen ytterligere informasjon om datasettet. Om vi derimot kvadrerer avvikene, unngår et uttrykk som alltid blir lik 0.
}
\end{document}
