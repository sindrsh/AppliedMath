...
Telefoner med smartfunksjoner (app-baserte) kom på det norske markedet i 2009. Tabellen\footnote{Tallene er hentet fra \net{https://www.medienorge.uib.no/statistikk/medium/ikt/405}{medienorge.uib.no}.
.} under viser det totale salget mobiltelefoner i tidsperioden 2009-2014.
\begin{center}
...
Skal vi presentere vår undersøkelser, bør vi vise datasett slik at det er lett for andre å se hva vi har funnet. Dette kan vi gjøre blant annet ved hjelp av \textit{frekvenstabeller}, \textit{søylediagram}, \textit{sektordiagram} eller \textit{linjediagram}.
...
\begin{enumerate}
	...
	\item Differansen mellom medianene fra punkt 3 er kvartil-bredden til det opprinnelige datasettet.
\end{enumerate}
...
\textbf{Undersøkelse 4} \os
\begin{itemize}
	\item For mobiltelefoner er kvartilbredden 
	\item For mobiltelefoner uten smartfunksjoner er kvartilbredden
	\item For mobiltelefoner med smartfunksjoner er kvartilbredden
\end{itemize}
} \vsk

\reg[Varians]{
Kvadrer differansene mellom hver enkelt verdi og gjennomsnittet til datasettet, og summer disse. Divider med antallet verdier i datasettet. \vsk

Differansen mellom en verdi og gjennomsnittet kalles \textit{avviket} til verdien. 
}
\eks{
Gitt datasettet 
\[\color{blue} 2\quad 5\quad 9\quad 6\quad 8 \]
Da har vi at
\[ \text{gjennomsnitt}=\frac{2+5+9+6+8}{5}=\colr{6} \]
Og videre er
\alg{
...
\begin{center}
	...
	\begin{tabular}{|c|c|c|}
		\hline
		Antall epler & Frekvens& Kvadrert avvik\\ \hline
		0 & 2 &$ \left(0-\frac{73}{15}\right)^2 $ \\
		1 & 2 &$ \left(1-\frac{73}{15}\right)^2 $\\
		4 & 3 &$ \left(4-\frac{73}{15}\right)^2 $\\
		5 & 2&$ \left(5-\frac{73}{15}\right)^2 $\\
		6 & 2 &$ \left(6-\frac{73}{15}\right)^2 $\\
		7 & 1 &$ \left(7-\frac{73}{15}\right)^2 $\\
		8 & 2 &$ \left(8-\frac{73}{15}\right)^2 $\\
		14& 1 &$ \left(9-\frac{73}{15}\right)^2 $\\ \hline 
		& sum & ?? \\ \hline
	\end{tabular}
\end{center}
}
\info{Tips}{Det kan være fristende å bruke det rangerte datasettene, men dette øker sjansen for følgefeil.}
\end{document}
